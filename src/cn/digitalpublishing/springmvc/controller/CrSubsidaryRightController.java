...
import cn.digitalpublishing.springmvc.form.product.PProductStructureRelationshipForm;
...
public class CrSubsidaryRightController extends BaseController {
	...
	public CrSubsidaryRightForm manager(CrSubsidaryRightForm form) throws Exception {
		...
		try {
			...
			if(!Strings.isNullOrEmpty(form.getLevel())) {
				condition.put("level", form.getLevel());
			}
			
			if(!Strings.isNullOrEmpty(form.getLicenseTypeName())) {
				condition.put("licenseTypeName", form.getLicenseTypeName());
			}
	...
	public ModelAndView edit(CrSubsidaryRightForm form) throws Exception {
		...
		try {
			...
			if (id != null && !"".equals(id)) {
				...
				if (subsidaryRight.getProduct() != null && !Strings.isNullOrEmpty(subsidaryRight.getProduct().getId())) {
					condition.clear();
		            condition.put("productId", subsidaryRight.getProduct().getId());
		            ...
		            condition.put("status", DicCache.getIdByCode(DicConstants.DIC_STATUS, DicConstants.DATA_STATUS_AVAILABLE));
		            List<PStructureType> pstructureTypeList = this.structureTypeService.getList(condition);
		            form.setPstructureTypeList(pstructureTypeList);
				}
