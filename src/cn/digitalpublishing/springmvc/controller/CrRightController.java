...
public class CrRightController extends BaseController {
	...
	public CrRightForm manager(CrRightForm form) throws Exception {
		...
		try {
			...
			// 授权开始时间
			condition.put("rlicenseStarton", crRight.getRlicenseStarton());
			
			// 授权结束时间
			condition.put("rlicenseEndon", crRight.getRlicenseEndon());
	...
	public ModelAndView edit(CrRightForm form) throws Exception {
		...
		try {
			...
			List<PProposal> proposalList = this.proposalService.getPProposalList(condition, "");
			form.setProposalList(proposalList);
	...
	public CrRightForm editSubmit(CrRightForm form) throws Exception {
		...
		try {
			...
			if (Strings.isNullOrEmpty(crRight.getProposal().getId())) {
				crRight.setProposal(null);
			}
