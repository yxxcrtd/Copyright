...
public class CrRlOwnerRoyaltyController extends BaseController {
	...
	public CrRlOwnerRoyaltyForm manager(CrRlOwnerRoyaltyForm form) throws Exception {
		...
		try {
			...
			if(!Strings.isNullOrEmpty(form.getRoyaltyType())) {
				condition.put("royaltyType", "%" + form.getRoyaltyType() + "%");
			}
			
			if(!Strings.isNullOrEmpty(form.getPriceType())) {
				condition.put("priceType", "%" + form.getPriceType() + "%");
			}
			
			if(!Strings.isNullOrEmpty(form.getTaxDescription())) {
				condition.put("taxDescription", "%" + form.getTaxDescription() + "%");
			}
	...
	public ModelAndView edit(CrRlOwnerRoyaltyForm form) throws Exception {
		...
		try {			
			...
			model.put("dicJson", objectMapper.writeValueAsString(form.getDic()));
		} catch (Exception e) {
			form.setMsg(exMsg(e));
		}
		model.put("form", form);
