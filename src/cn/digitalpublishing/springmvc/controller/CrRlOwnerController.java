	...
	public CrRlOwnerForm manager(CrRlOwnerForm form) throws Exception {
		...
		try {
			...
			if (!Strings.isNullOrEmpty(form.getName())) {
				condition.put("owner_person_name", "%" + form.getName() + "%");
			}
	...
	public ModelAndView edit(CrRlOwnerForm form) throws Exception {
		...
		try {			
			...
			if (!Strings.isNullOrEmpty(crOwner.getRlownerId())) {
				...
				CrRlProduct rlProduct = this.crProductService.getById(crOwner.getRlProduct().getRlpId());
				PProduct product = this.productService.getProduct(rlProduct.getProduct().getId());
				this.crOwnerService.edit(product.getId(),crOwner, form.getRight(), form.getDic());
