...
public class CrRlProductController extends BaseController {
	...
	public CrRlProductForm manager(CrRlProductForm form) throws Exception {
		...
		try {
			...
			if (!Strings.isNullOrEmpty(form.getProductCode())) {
				condition.put("productCode", "%" + form.getProductCode() + "%");
			}
			
			if (!Strings.isNullOrEmpty(form.getProductTitle())) {
				condition.put("productTitle", "%" + form.getProductTitle() + "%");
			}
	...
	public CrRlProductForm delete(CrRlProductForm form) throws Exception {
		CrRlProduct crProduct = form.getCrProduct();
		try {
			this.crProductService.delete(crProduct.getRlpId());
			form.setIsSuccess(SUCCESS);
	...
	public ModelAndView selectProduct(CrRlProductForm form) throws Exception {
