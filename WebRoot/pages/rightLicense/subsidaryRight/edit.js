Editorial.SubsidaryRight.validate = function() {
    ...
    if (!Editorial.SubsidaryRight.validateLevel()) {
    	...
    	if (text == "授权级别") {
    		...
    		if ($("#royaltyRule").val() != null) {
    			var royaltyRule = $("#royaltyRule option:selected").text(); // 版税规则 文本
    			if (royaltyRule == "简单分割原则") {
    				
    			} else {
    				if (!Editorial.SubsidaryRight.validateAdjustRule()) {
    	    			alert(!Editorial.SubsidaryRight.validateAdjustRule());
    	    	        flag = false;
    	    	    }
    			}
    		}
    		...
    		if (!Editorial.SubsidaryRight.validateStructureType()) {
    		    	flag = false;
    		}
    	} else if (text == "素材级别") {
    		if (!Editorial.SubsidaryRight.validateStatus()) {
    ...
    $.ajax({
        ...
        success : function(response) {
            if (response.isSuccess == "true") {
                ...
                for ( var i = 0; i < response.pstructureTypeList.length; i++) {
                    $("#structureType").append("<option value='" + response.pstructureTypeList[i].id + "'>" + response.pstructureTypeList[i].name + "</option>");
                }
            }
        },
 ...
 * 附属权利级别切换
 */
Editorial.SubsidaryRight.levelToggle = function() {
	var text = $("#level").find("option:selected").text();
    clearMessage();
    if (text == "授权级别") {
    	$("#productDiv").hide();
    	$("#personTypeRelationshipDiv").hide();
    	$("#structureTypeDiv").hide();
    	$("#structureNameDiv").hide();
    	
    	
    } else if (text == "产品级别") {
    	$("#productDiv").show();
    	$("#personTypeRelationshipDiv").show();
    	$("#structureTypeDiv").hide();
    	$("#structureNameDiv").hide();
    } else if (text == "素材类型级别") {
    	$("#productDiv").show();
    	$("#personTypeRelationshipDiv").show();
    	$("#structureTypeDiv").show();
    	$("#structureNameDiv").hide();
    } else if (text == "素材级别") {
    	$("#productDiv").show();
    	$("#personTypeRelationshipDiv").show();
    	$("#structureTypeDiv").show();
    	$("#structureNameDiv").show();
    }
};
 ...
 * 调整规则切换
 */
Editorial.SubsidaryRight.adjustRuleToggle = function() {
	if ($("#adjustRule").val() != "") { // 调整规则
		if ($("#adjustRule option:selected").text() == "不上调") {
			$("#calculationRulesLi").hide(); // 计算规则 li
		} else {
			$("#calculationRulesLi").show(); // 计算规则 li
		}
	}
};
 ...
 * 版税规则切换
 */
Editorial.SubsidaryRight.royaltyRuleToggle = function() {
	if ($("#royaltyRule").val() != null) {
		var royaltyRule = $("#royaltyRule option:selected").text(); // 版税规则 文本
		if (royaltyRule == "简单分割原则") {
			$("#retainPercentDiv").show(); // 出版社比率
			$("#ownerPercentDiv").show(); // 贡献者比率
			$("#initRateDiv").show(); // 初始比率
			
			$("#adjustRuleDiv").hide(); // 调整规则
			$("#calculationRulesLi").hide(); // 计算规则 li
		} else {
			$("#initRateDiv").show(); // 初始比率
			$("#adjustRuleDiv").show(); // 调整规则
			
			$("#retainPercentDiv").hide(); // 出版社比率
			$("#ownerPercentDiv").hide(); // 贡献者比率
			
			Editorial.SubsidaryRight.adjustRuleToggle();
		}
	}
...
$(function() {
	...
	if ($("#level").val() != null) {
		Editorial.SubsidaryRight.levelToggle();
	}
