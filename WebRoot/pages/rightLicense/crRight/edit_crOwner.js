...
Editorial.CrOwner.removePrepaidAndExpenses = function(fixfeeId) {
		$.ajax({
			"dataType" : "json",
			"type" : "POST",
			"url" : $("#ctx").val() + "/pages/rightLicense/crOwnerFee/form/delete",
			"data" : {
				"crOwnerFee.fixfeeId" : fixfeeId
			},
			"cache" : false,
			"success" : function(response) {
		...
		Editorial.CrOwner.prepaidAndExpensesDataTable = $("#prepaidAndExpensesDataTable").dataTable({
			...
			"aoColumns" : [{
				...
				"fnRender" : function(oObj) {
					...
					var removeBtn = "<button class='btn btn-mini btn-danger' title='删除' type='button' onclick=\"Editorial.CrOwner.removePrepaidAndExpenses('"+ oObj.aData.fixfeeId +"')\">";
...
Editorial.CrOwner.removeBeneficiary = function(feePayeeId) {
	$.ajax({
		"dataType" : "json",
		"type" : "POST",
		"url" : $("#ctx").val() + "/pages/rightLicense/crOwnerPayee/form/delete",
		"data" : {
			"crOwnerPayee.feePayeeId" : feePayeeId
		},
		"cache" : false,
		"success" : function(response) {
			if (response.isSuccess == "true") {
		...
		Editorial.CrOwner.beneficiaryDataTable = $("#beneficiaryDataTable").dataTable({
			...
			"aoColumns" : [{
				...
				"fnRender" : function(oObj) {
					...
					var removeBtn = "<button class='btn btn-mini btn-danger' title='删除' type='button' onclick=\"Editorial.CrOwner.removeBeneficiary('"+ oObj.aData.feePayeeId +"')\">";
					removeBtn += "<i class='icon-trash bigger-120'></i>";
					removeBtn += "</button>";
...
Editorial.CrOwner.removeRoyaltyCalculationRules = function(rloRoyaltyId) {
	$.ajax({
		"dataType" : "json",
		"type" : "POST",
		"url" : $("#ctx").val() + "/pages/rightLicense/crOwnerRoyalty/form/delete",
		"data" : {
			"crOwnerRoyalty.rloRoyaltyId" : rloRoyaltyId
		},
		"cache" : false,
		"success" : function(response) {
			if (response.isSuccess == "true") {
...
Editorial.CrOwner.query = function() {
	// 重新刷新页面Table
	refreshFrameDataTableInFrame('Editorial.CrOwner.royaltyCalculationRulesDataTable');
};
		...
		Editorial.CrOwner.royaltyCalculationRulesDataTable = $("#royaltyCalculationRulesDataTable").dataTable({
			...
			"aoColumns" : [{
				...
				"fnRender" : function(oObj) {
					...
					var removeBtn = "<button class='btn btn-mini btn-danger' title='删除' type='button' onclick=\"Editorial.CrOwner.removeRoyaltyCalculationRules('"+ oObj.aData.rloRoyaltyId +"')\">";
					removeBtn += "<i class='icon-trash bigger-120'></i>";
					removeBtn += "</button>";
