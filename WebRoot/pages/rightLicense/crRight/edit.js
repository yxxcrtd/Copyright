...
Editorial.CrRight.validate_paymentTrem = function() {
	...
	_obj.val(_obj.val().trim());
	if (_obj.val()) {
		if(!(/^[0-9]+$/).test($("#crRight_paymentTrem").val())){
			showValidateMsg(false, "只能输入数字！", _id);
			Editorial.CrRight.validate_flag = false;
			return false;
		}
		showValidateMsg(true, "可以使用", _id);
...
Editorial.CrRight.validate_rlicenseEndon = function() {
	...
	var _obj = $(_id);
	_obj.val(_obj.val().trim());
	if ($("#crRight_rlicenseEndon").val().trim()<$("#crRight_rlicenseStarton").val().trim()) {
		showValidateMsg(false, "授权结束时间要晚于授权开始时间！", _id);
		Editorial.CrRight.validate_flag = false;
	} else if(_obj.val()){
		showValidateMsg(true, "可以使用", _id);
	}else {
...
Editorial.CrRight.validate_start = function() {
	...
	if (_obj.val()) {
		...
		showValidateMsg(false, "授权开始依据不能为空！", _id);
		Editorial.CrRight.validate_flag = false;
	}
};
...
Editorial.CrRight.queryProducts = function() {
	initPagingParamsInFrame('Editorial.CrRight.signedProductsDataTable');
	// 重新刷新页面Table         
		...
		Editorial.CrRight.signedProductsDataTable = $("#signedProductsDataTable").dataTable({
			...
			"aoColumns" : [{
				...
				"fnRender" : function(oObj) {
					...
					var removeBtn = "<button class='btn btn-mini btn-danger' title='删除' type='button' onclick=\"Editorial.CrRight.removeSignedProduct('"+ oObj.aData.rlpId +"')\">";
...
Editorial.CrRight.removeSignedProduct = function(id) {
	openConfirmModalInFrame("您确定删除该签约产品吗？", function() {
		$.ajax({
			"dataType" : "json",
			"type" : "POST",
			"url" : $("#ctx").val() + "/pages/rightLicense/crProduct/form/delete",
			"data" : {
				"crProduct.rlpId" : id
			},
			"cache" : false,
...
Editorial.CrRight.queryContributor = function() {
	initPagingParamsInFrame('Editorial.CrRight.signingContributorDataTable');
	// 重新刷新页面Table         
		...
		Editorial.CrRight.signingContributorDataTable = $("#signingContributorDataTable").dataTable({
			...
			"aoColumns" : [{
				...
				"fnRender" : function(oObj) {
					...
					var removeBtn = "<button class='btn btn-mini btn-danger' type='button' title='删除' onclick=\"Editorial.CrRight.removeSigningContributor('"+ oObj.aData.rlownerId +"')\">";
					removeBtn += "<i class='icon-trash bigger-120'></i>";
					removeBtn += "</button>";
...
Editorial.CrRight.removeSigningContributor = function(id) {
	openConfirmModalInFrame("您确定删除该签约贡献者吗？", function() {
		$.ajax({
			"dataType" : "json",
			"type" : "POST",
			"url" : $("#ctx").val() + "/pages/rightLicense/crOwner/form/delete",
			"data" : {
				"crOwner.rlownerId" : id
			},
			"cache" : false,
			"success" : function(response) {
				if (response.isSuccess == "true") {
...
Editorial.SubsidaryRight.query = function() {
	initPagingParamsInFrame('Editorial.CrRight.subsidiaryRightsDataTable');
	// 重新刷新页面Table         
	...
	if(Editorial.CrRight.subsidiaryRightsDataTable == null) {
		Editorial.CrRight.subsidiaryRightsDataTable = $("#subsidiaryRightsDataTable").dataTable({
			...
			"fnServerParams" : function(aoData) {
				...
				aoData.push({
					"name" : "level", "value" : $("#query_level").val()
				});
				aoData.push({
					"name" : "licenseTypeName", "value" : $("#query_licenseType").val()
				});
			...
			"aoColumns" : [ {
				...
				"fnRender" : function(oObj) {
					...
					var trash = '<button class="btn btn-mini btn-danger" title="删除" type="button" onclick="Editorial.CrRight.removeSubsidiaryRights(\'' + oObj.aData.srlicenseId + '\')"><i class="icon-trash bigger-120"></i></button>';
					var end = '</div>';
...
Editorial.CrRight.removeSubsidiaryRights = function(id) {
	openConfirmModalInFrame("您确定删除该附属版权吗？", function() {
		$.ajax({
			"dataType" : "json",
			"type" : "POST",
			"url" : $("#ctx").val() + "/pages/rightLicense/subsidaryRight/form/delete?id="+id,
			"data" : {
				"obj.id" : id
			},
			"cache" : false,
