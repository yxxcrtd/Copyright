...
Editorial.CrOwnerPayee.validate = function() {
	...
	var text = $("#crOwnerPayee_status").find("option:selected").text();
	if (text == "失效") {
        Editorial.CrOwnerPayee.validate_feePayeeReason();
    }
...
Editorial.CrOwnerPayee.validate_status = function() {
	...
	if (_obj.val()) {
		...
		if (text == "失效") {
            ...
            $("#crOwnerPayee_feePayeeReason").val("");
        }
		showValidateMsg(true, "可以使用", _id);
	} else {
...
Editorial.CrOwnerPayee.validate_feePayeeReason = function() {
	var _id = "#crOwnerPayee_feePayeeReason";
	var _obj = $(_id);
	_obj.val(_obj.val().trim());
	if (_obj.val()) {
		showValidateMsg(true, "可以使用", _id);
	} else {
		showValidateMsg(false, "失效原因不能为空！", _id);
		Editorial.CrOwnerPayee.validate_flag = false;
	}
};

Editorial.CrOwnerPayee.statusToggle = function() {
	var text = $("#crOwnerPayee_status").find("option:selected").text();
	if (text == "有效") {
	    $("#crOwnerPayee_feePayeeReason_div").hide();
	}else {
		$("#crOwnerPayee_feePayeeReason_div").show();
	} 
};
