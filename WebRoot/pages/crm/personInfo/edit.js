...
Editorial.CRM.PersonInfo.validate = function() {
	...
	if (!Editorial.CRM.PersonInfo.validateEmailLength()) {
		flag = false;
	}
	if($("#relation_id").val() == "") {
...
Editorial.CRM.PersonInfo.validateEmailLength = function() {
	...
	if ($("#email").val().trim() == "") {
		...
		$("#emailDiv").removeClass("error").addClass("success");
		$("#emailSpan").html("通过验证");
		return true;
	}	
...
function onCheck(e, treeId, treeNode) {
	...
	$("#corpName").val((treeNode.name).stripHTML());
	
	Editorial.CRM.PersonInfo.validateCorp();
	return;
...
function showMenu() {
	/*var setting = {
		...
		$.ajax({
			"dataType" : "json",
			"type" : "POST",
			"url" : $("#ctx").val() + "/pages/crmCorp/form/getAllTreeNodes?corpType.code=org",
			...
			"success" : function(response) {
				...
				var setting2 = {
					...
					$.fn.zTree.init($("#treeManage"), setting2, response);
					$.fn.zTree.getZTreeObj("treeManage").expandAll(true);
