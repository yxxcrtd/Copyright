...
Editorial.Proposal.delObj = function(id) {
	openConfirmModalInFrame("您确定执行删除该策划创意的操作吗？", function() {
		var url = $('#ctx').val() + "/pages/product/proposal/form/delete?id=" + id;
		$.ajax({
			"dataType" : 'json',
			"type" : "POST",
			"url" : url,
...
$(function () {
    ...
    Editorial.Proposal.oTable1 = $('#table_report').dataTable({
        ...
        "sAjaxSource": $('#ctx').val() + "/pages/product/proposal/form/manager", //mvc后台ajax调用接口。
        ...
        "aoColumns": [
            ...
            },
            {
                ...
                "fnRender": function (oObj) {
                    ...
                    var del = '<button class="btn btn-mini btn-danger" title="删除策划创意" data-toggle="tooltip" data-placement="bottom" onclick="Editorial.Proposal.delObj(\'' + oObj.aData.id + '\')"><i class="icon-trash bigger-120"></i></button>';
                    var end = '</div>';
                    return start + del + end;
                }
            }
        ],
        //多语言配置
