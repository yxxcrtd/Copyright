...
//查看贡献者信息
Editorial.Product.ProductInfo.personObj = function(id) {
	var url = $('#ctx').val() + "/pages/productPersonRelationship/form/addPerson?id=" + id;
	...
	Editorial.Product.ProductInfo.oTable1 = $('#table_report').dataTable({
		...
		"aoColumns" : [ {
			...
			"fnRender" : function(oObj) {
				...
				var person = '<button class="btn btn-mini btn-success" title="添加贡献者" onclick="Editorial.Product.ProductInfo.personObj(\'' + oObj.aData.id + '\')"><i class="icon-plus-sign bigger-120"></i></button>';
				...
				var end = '</div>';
				return start + person + edit + trash + splitXML + viewElement + end; 
			}
		} ],
